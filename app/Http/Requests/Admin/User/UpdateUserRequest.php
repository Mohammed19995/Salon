...
class UpdateUserRequest extends BaseRequest
{
    ...
    {
        $check_phone = $this->checkPhone('users',false , $this->request->get('id') , $this->request->get('phone_code') , $this->request->get('phone'));
//        $check_admin_phone = $this->checkPhone('admins',true , -1 , $this->request->get('phone_code') , $this->request->get('phone'));

        $rules =  [
            'name'          => ['required', 'max:250'],
            'email'         => ['required', 'email', 'max:250',
                Rule::unique('users', 'email')->ignore($this->request->get('id'))
